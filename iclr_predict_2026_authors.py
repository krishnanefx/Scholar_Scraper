...
def main():
    print("=== ICLR 2026 Author Participation Prediction ===")
    
    # Load the data directly from parquet
    print("Loading ICLR dataset from parquet...")
    df = pd.read_parquet('iclr_2020_2025_combined_data.parquet')
    ...
    output_file = 'iclr_2026_predictions.csv'
    results_to_save.to_csv(output_file, index=False)
    
    print(f"\n=== ICLR 2026 Predictions Complete ===")
    ...
    # Save the model
    joblib.dump(ensemble_final, 'iclr_participation_model.pkl')
    print(f"\nModel saved as: iclr_participation_model.pkl")
