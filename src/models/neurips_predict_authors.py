...
def main():
    ...
    print(f"Loaded {len(df)} records from NeurIPS parquet dataset at {data_path}")
    
    # Determine prediction year dynamically
    max_year = df['Year'].max()
    prediction_year = max_year + 1
    print(f"=== NeurIPS {prediction_year} Author Participation Prediction ===")
    ...
    print(f"  • Historically, ~4-6% of eligible authors participate in subsequent NeurIPS conferences")
    ...
    print(f"  • High historical participation rate (>4 papers per year)")
    print(f"  • Strong co-authorship networks with other frequent participants")
    print(f"  • Consistent publication pattern in NeurIPS venues")
    ...
    for i, row in top_10.iterrows():
        ...
        reasons = []
        if row['num_participations'] >= 15:
            reasons.append(f"highly prolific author ({row['num_participations']} papers)")
        elif row['num_participations'] >= 10:
        ...
        if row['participation_rate'] >= 4:
            reasons.append(f"very high productivity ({row['participation_rate']:.1f} papers/year)")
        elif row['participation_rate'] >= 2.5:
    ...
    print(f"\n=== NeurIPS Prediction Pattern Analysis ===")
    ...
    print(f"  • Highly prolific authors (≥15 papers): {(predicted['num_participations'] >= 15).sum()} ({(predicted['num_participations'] >= 15).sum() / len(predicted) * 100:.1f}%)")
